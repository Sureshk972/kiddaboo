# Kiddaboo Regression Run — 2026-05-09

Driver: Suresh (host account, Chrome)
Site: https://kiddaboo.com
Build at start of run: f37795e (badge-refresh fix on top of admin protection / RSVP children gate / RLS UPDATE policy)
...
| 2. Onboarding | — | 0 | 0 | 0 |
| 3. Browse | — | 0 | 0 | 0 |
| 4. Playgroup Detail | — | 0 | 0 | 0 |
| 5. My Groups | — | 0 | 0 | 0 |
| 6. Messages | — | 0 | 0 | 0 |
| 7. Profile & Settings | — | 0 | 0 | 0 |
| 8. Premium & Payments | — | 0 | 0 | 0 |
| 9. Host Flow | 15 | 0 | 0 | 0 |
| 10. Cross-Cutting | — | 0 | 0 | 0 |
| 11. Security | — | 0 | 0 | 0 |
| 12. Vouching & Trust | — | 0 | 0 | 0 |
| **Total** | **~200** | **0** | **0** | **0** |

## Failures / Fix Queue

_(Build-and-hold — fixes committed locally on `main`, not pushed.)_

### VFY-01 — Verification request cycle (request → submit → pending → ...) — **PASS**
Confirmed end-to-end by Suresh on live site.

### VFY-02 — Star icon shown next to "Request verification" CTA conflicts with the unrequested state
- **Symptom:** Gold star icon (which signals "verified") rendered alongside the "Request verification" button — created a contradictory visual.
- **Fix:** local commit `e6b7138` — icon is now state-aware (shield default, clock pending, caution triangle rejected, star sage when verified).
- **Status:** Fixed locally, queued for next deploy.

### NTF-01 — Session Reminders toggle locked for host-premium accounts
- **Symptom:** As host-premium subscriber, Notifications → Session Reminders toggle is off and uninteractable; "Upgrade to enable" link appears though the user is already premium.
- **Root cause:** `NotificationSettings.jsx` used `isPremium` from `useSubscription`, which is joiner-only (`!!joinerSub`). Host-premium was ignored.
- **Fix:** local commit `b407b6e` — read both `isPremium` and `isHostPremium`, unlock when either is active.
- **Status:** Fixed locally, queued for next deploy.
